...
def page_admin_dashboard():
    ...
    courses = get_courses()
    if not courses:
        st.info("No courses available. Please add a course first.")
    else:
...
<div class="course-title">{course[1]}</div>
<div class="course-subtitle">{course[2]}</div>
<div class="course-desc">{course[3][:200]}...</div>
<p><b>Price:</b> {"Free" if course[4]==0 else f"₹{course[4]:,.0f}"}</p>
</div>
            """, unsafe_allow_html=True)

            # Update Course
            with st.expander("Update Course"):
                with st.form(f"update_course_form_{course[0]}"):
                    ...
                    if st.form_submit_button("Update Course", key=f"update_{course[0]}"):
            ...
            with st.expander("Add Module"):
                with st.form(f"add_module_form_{course[0]}"):
                    ...
                    if st.form_submit_button("Add Module", key=f"addmod_{course[0]}"):
